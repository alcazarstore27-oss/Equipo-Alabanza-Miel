...
saveBtn.onclick = () => {
  ...
  // ===== QUITAR SERVICIO ANTERIOR =====
  if (previousService) {

    const oldService =
      services.find(
        s => s.id === previousService
      );

    if (oldService) {

      oldService.order =
        oldService.order.filter(
          id => id !== song.id
        );
    }
  }
  ...
  if (song.serviceId) {
    ...
    if (
      service &&
      !service.order.includes(song.id)
    ) {
...
deleteServiceBtn.onclick = () => {
  ...
  songs.forEach(song => {
    ...
    if (
      song.serviceId === currentServiceId
    ) {
